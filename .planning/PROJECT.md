...
---
*Last updated: 2026-05-24 — Phase 03 (lava-top-plans-catalog) complete. All 16 payment requirements (PAY-01..PAY-16) landed: migrations 019+020+021, dynamic plans catalog (PlanLimits map deleted), pure internal/lava/ HTTP client, public GET /plans + JWT plan_id, admin CRUD (13 handlers), webhook handler with X-Api-Key constant-time verify + CIDR allowlist + UNIQUE-constraint idempotency + 5-event dispatch, expiry-downgrade scheduler, admin-web Plans UI, docs + sandbox smoke. Code review: 9/10 Critical+Warning findings fixed (CR-01 TrustedProxies, CR-02 idempotency UNIQUE NULL hole + test schema drift, WR-01..WR-04, WR-06..WR-08); WR-05 admin-web cross-tab refresh deferred (requires design decision). Stripe code/refs fully removed from production paths (Stripe-Go dep still in go.mod for Phase 8 cleanup). HUMAN_NEEDED: end-to-end lava sandbox test requires LAVA_API_KEY_SANDBOX. Phase 4 (landing surfaces) unblocked.*
