...
- [x] Sign in with Apple + Sign in with Google on backend (`/auth/apple`, `/auth/google`)
- [x] Account-linking by verified email; reject `@privaterelay.appleid.com` from linking
- [x] Lava.top HTTP client (`internal/lava/`) + checkout endpoint (`POST /checkout`)
- [x] Lava.top webhook handler with strict idempotency, IP allowlist via Fiber `TrustedProxies`, constant-time `X-Api-Key` check
- [x] Dynamic plans catalog (`plans`, `plan_servers`, `plan_offers` tables) replacing hardcoded `PlanLimits` map
- [x] Server access enforcement at repository layer via `plan_servers` join (admin bypass)
- [x] Admin CRUD for plans / offers / plan-servers (`/admin/plans/*`)
- [x] Public `GET /api/v1/plans` for landing /pricing dynamic rendering
- [x] Landing site: `/login`, `/dashboard`, `/pricing`, `/pay/success` (polls invoice as defense in depth), `/pay/fail`
...
- [x] Admin panel overhaul: KPI dashboard (MRR / paid users / churn), user controls (suspend, force-Pro, force-disconnect), webhook log + replay, system controls (feature flags, maintenance mode, broadcast), readyz/livez
- [x] Cleanup: delete Stripe code, per-user VLESS UUID rotation, refresh tokens → 32-byte opaque, security headers middleware, govulncheck in CI — **Phase 8 complete (2026-06-02). All 17 HARD-XX requirements landed across 9 plans in 3 waves (Nyquist RED test infra → audit fixes [Telegram private-chat gate, admin prefix-search, role-change audit diff, /health version-leak] + API hardening [admin security headers, zap token redaction, bcrypt 12, fail-closed link limiter, /debug/error bucket, per-user HMAC server order] + govulncheck CI gate + useVpnConnection refactor → opaque/device-bound refresh tokens (migration 025) + Stripe removal with durable `TestNoStripeReferences` fence → per-user VLESS UUID rotation with tunnel wire enforcement (migration 026) + mobile Keychain/EncryptedSharedPreferences token storage). Verification 8/8 criteria passed; API+tunnel build/vet clean; full suite green except 3 documented pre-existing base failures. Code review 0 Critical / 5 Warning / 9 Info — all 5 warnings fixed inline (WR-02 tunnel empty-set outage guard, WR-03 partial-UNIQUE one-active-UUID-per-user, WR-01 device-binding warn, WR-04 CancelSubscription lock, WR-05 rate-limit expiry). HUMAN_NEEDED: GitHub branch-protection toggle (HARD-09) + on-device Keychain verification & single coordinated re-login (HARD-16) tracked in `08-HUMAN-UAT.md`. Security gate: `/gsd-secure-phase 08` not yet run.**
...
---
*Last updated: 2026-06-03 — **🎉 Milestone v2.2.0 "Lava + SSO" SHIPPED** (tagged v2.2.0). 8 phases / 71 plans / 153 tasks over 2026-05-22→06-03. The core value is delivered end-to-end: sign in once with Apple/Google → pay on risevpn.com via lava.top → Pro unlocks on every device, with per-user VLESS wire enforcement, opaque device-bound tokens, full Stripe removal, and a hardened admin panel. All 8 phases verified + `nyquist_compliant`; all 82 requirements satisfied. The milestone audit caught + fixed 2 cross-phase gaps before completion (critical mobile-SSO camelCase/snake_case contract break; VLESS-revoke on admin cancel). Remaining before GA = operator HUMAN-UAT only (live Apple/Google OAuth, lava.top sandbox, on-device Keychain, ~10k load test, govulncheck branch-protection toggle) across 6 `*-HUMAN-UAT.md`. Full requirements archived to `milestones/v2.2.0-REQUIREMENTS.md`; roadmap to `milestones/v2.2.0-ROADMAP.md`. Next: `/gsd-new-milestone`. — Prior phase-by-phase detail below.*

*Prior update: 2026-06-02 — Phase 08 (cleanup-hardening) complete. All 17 HARD-XX requirements landed across 9 plans in 3 wave-parallel waves: Wave 1 (Nyquist RED test infra; audit fixes — Telegram private-chat gate, admin prefix-search + index migration 027, role-change audit diff, /health version-leak; API hardening — admin security headers, zap token redaction, bcrypt 12, fail-closed link limiter, /debug/error bucket, per-user HMAC server order; govulncheck CI gate; useVpnConnection 591→78 refactor), Wave 2 (opaque 32-byte device/IP-bound refresh tokens + migration 025 clean-break session wipe; full Stripe removal + durable `TestNoStripeReferences` fence), Wave 3 (per-user VLESS UUID allocation/rotation with tunnel-side wire enforcement + migration 026; mobile Keychain/EncryptedSharedPreferences token storage + device_id-on-refresh). Verification 8/8 criteria passed; API+tunnel build/vet clean; full `server/api` suite green except 3 documented PRE-EXISTING base failures (TestMigrations019_020, TestPerfIndexes, TestFlushHeartbeatsCollapsesNto1 — all fail at base 2122b84, none wave-touched; see `deferred-items.md`). Wave-parallel execution recovered 1 mid-run agent socket failure (08-01 retried) and resolved cross-worktree duplicate-test merge conflicts + 1 flawed RED-test harness (HARD-06 capture). Code review 0 Critical / 5 Warning / 9 Info — ALL 5 warnings fixed inline (WR-02 tunnel empty-set outage guard, WR-03 partial-UNIQUE one-active-UUID-per-user, WR-01 device-binding warn, WR-04 CancelSubscription WithUserLock, WR-05 rate-limit token-expiry). HUMAN_NEEDED: GitHub branch-protection toggle (HARD-09) + on-device Keychain verification & single coordinated re-login (HARD-16) tracked in `08-HUMAN-UAT.md`. Security gate: `/gsd-secure-phase 08` not yet run (security_enforcement on). This is the LAST phase of milestone v2.2.0.*
*Prior update: 2026-05-30 — Phase 06 (performance-scalability) complete. All 9 PERF-XX requirements landed across 8 plans (06-00 RED-first test infra → 06-01 data-tier compose split + Fiber/PG/Redis tuning → 06-02 perf indexes 022/023 → 06-03 `/servers` Redis cache → 06-04 user existence+tier cache for AuthRequired → 06-05 heartbeat→Redis 10s bulk flush + `RUN_SCHEDULER` gate + weekly prune → 06-06 `ctx` propagation through all repos → 06-07 prod deploy runbook + checkpoint). Verification 9/9 passed; full `server/api` build+vet+short suite green. Wave-parallel execution recovered 2 mid-run agent failures (06-00 socket crash, 06-06 stall) + 1 missed ctx call site caught by post-merge build + 1 scheduler cleanup-on-start regression fixed. Code review 0 Critical / 4 Warning / 5 Info — WR-03 (expiry-downgrade renewal race on the money path) fixed inline; WR-01/WR-02/WR-04 + Info remain advisory. HUMAN_NEEDED / DEFERRED: live ~10k load test + physical second-host PG/Redis move + live-DB index backfill (PENDING-OPERATOR — empty-volume initdb caveat) tracked in `06-HUMAN-UAT.md` + `docs/runbooks/06-perf-deploy-runbook.md`. Security gate: `/gsd-secure-phase 06` not yet run (security_enforcement on; threat model has T-06-USERCACHE EoP / T-06-REDISOOM / T-06-DATALINK / T-06-BACKFILL-GAP). Next: Phase 7 (admin-panel-overhaul) — not yet planned.*
