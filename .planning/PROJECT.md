...
---
*Last updated: 2026-05-29 — Phase 05 (mobile-sso-pro-cta) complete. All 7 APP-XX requirements landed across 5 plans (05-00 test scaffolding → 05-01 native config → 05-02 services → 05-03 UI → 05-04 release prep) + a review-fix pass (6 fixes, incl. the launch-critical deep-link parser bug WR-01, locked with 30 regression tests; suite 10 suites/75 tests green, tsc clean). Apple/Google/Guest LoginScreen, informational PaymentScreen (no IAP), vpnapp:// deep-link → ActivatingProModal polling, guest→SSO in-place promotion, app at 2.2.0, signed .aab built (JDK 17). Security: 0 Critical/High introduced; M-2 cleartext + L-4 AsyncStorage deferred to Phase 8. HUMAN_NEEDED: live SSO auth + physical-device + TestFlight/Play uploads deferred to release phase per operator (placeholder OAuth creds DEF-05-CREDS). Next: Phase 6 (performance-&-scalability) — not yet planned.*
