...
|-----|----|---------|-----------------|-----------|-------------------|-------------|--------|
| HARD-01 | SC#1 | Go API | `grep -rn stripe server/ --include=*.go` == 0; `stripe_id` column absent | shell + integration | `grep -rn stripe server/ --include=*.go` (expect 0); existing `migrations_test.go` asserts column absence | ✅ assert / ❌ W0 grep gate | ⬜ pending |
...
| HARD-09 | SC#3 | CI | vuln-introducing PR unmergeable | CI check | PR adding known-vuln dep → `govulncheck-action` job red; merge-block via GitHub branch-protection (manual) | ❌ W0 (+manual) | ⬜ pending |
...
- [ ] grep-stripe shell assertion in CI or test (SC#1 literal gate)
